...
def solution(dimensions, your_position, trainer_position, distance):
    ...
    def get_reflected_pos(dimensions, pt, refl_lvl):
        x = pt[0] + 2 * (
            (dimensions[0] - pt[0]) * math.ceil(refl_lvl[0] / 2.0)
            + (pt[0]) * math.floor(refl_lvl[0] / 2.0)
        )
        y = pt[1] + 2 * (
            (dimensions[1] - pt[1]) * math.ceil(refl_lvl[1] / 2.0)
            + (pt[1]) * math.floor(refl_lvl[1] / 2.0)
        )
        return [x, y]
