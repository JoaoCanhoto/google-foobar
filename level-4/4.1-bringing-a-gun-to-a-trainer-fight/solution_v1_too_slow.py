...
def solution(dimensions, your_position, trainer_position, distance):
    ...
    def create_reflections(level=1):  # return list of different possible reflections
        return [
            (x, y)
            for y in range(-level, level + 1, 1)
            for x in range(-level, level + 1, 1)
            if (abs(x) == level) or (abs(y) == level)
        ]
    ...
    def get_reflected_pos(dimensions, pt, refl_lvl):
        x = pt[0] + (
            2 * (dimensions[0] - pt[0]) * math.ceil(refl_lvl[0] / 2.0)
            + 2 * (pt[0]) * math.floor(refl_lvl[0] / 2.0)
        )
        y = pt[1] + (
            2 * (dimensions[1] - pt[1]) * math.ceil(refl_lvl[1] / 2.0)
            + 2 * (pt[1]) * math.floor(refl_lvl[1] / 2.0)
        )
        return [x, y]
    ...
    def get_direction(p1, p2):
        ...
        return [v_x / gcd, v_y / gcd] if gcd != 0 else [v_x, v_y]

    def check_dir_unavailability(dir_trainer, list_of_dir_unavailable):
        return dir_trainer in list_of_dir_unavailable

    def append_direction(dir_p, list_of_dir_unavailable):
        if dir_p not in list_of_dir_unavailable:
            list_of_dir_unavailable.append(dir_p)
        return list_of_dir_unavailable
    ...
    dir_to_trainer = get_direction(your_position, trainer_position)

    list_of_dir_unavailable = [dir_to_trainer]
        ...
        for refl in list_refl:
            ...
            if dist_trainer > distance:
                if get_distance(your_position, refct_my_pos) < distance:
                    ...
                    list_of_dir_unavailable = append_direction(
                        dir_my_refct, list_of_dir_unavailable
                    )
                continue
            ...
            if check_dir_unavailability(dir_trainer, list_of_dir_unavailable):
                continue

            list_of_dir_unavailable = append_direction(
                dir_trainer, list_of_dir_unavailable
            )
            count_shots += 1
