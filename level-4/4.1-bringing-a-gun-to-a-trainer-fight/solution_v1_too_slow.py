...
def solution(dimensions, your_position, trainer_position, distance):
    ...
    def get_reflected_pos(dimensions, pt, refl_lvl):
        x = pt[0] + \
            (2 * (dimensions[0] - pt[0]) * math.ceil(refl_lvl[0] / 2.) + 2 * (pt[0]) * math.floor(refl_lvl[0] / 2.))
        y = pt[1] + \
            (2 * (dimensions[1] - pt[1]) * math.ceil(refl_lvl[1] / 2.) + 2 * (pt[1]) * math.floor(refl_lvl[1] / 2.))
        return [x, y]
        ...
        for refl in list_refl:
            ...
            if dist_trainer > distance:
                if get_distance(your_position, refct_my_pos) < distance:
                    ...
                    list_of_dir_unavailable = append_direction(dir_my_refct, list_of_dir_unavailable)
                continue
            ...
            list_of_dir_unavailable = append_direction(dir_trainer, list_of_dir_unavailable)
            count_shots += 1
