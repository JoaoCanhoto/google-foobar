...
def solution(dimensions, your_position, trainer_position, distance):
    ...
    def get_reflected_pos(dimensions, pt, refl_lvl):
        x = dimensions[0] * refl_lvl[0] + (dimensions[0] - pt[0] if refl_lvl[0] % 2 else pt[0])
        y = dimensions[1] * refl_lvl[1] + (dimensions[1] - pt[1] if refl_lvl[1] % 2 else pt[1])
        return [x, y]
    ...
    def append_direction_with_dist(dir_pt, matrix, d):
        x_pos = dir_pt[0] + x_dir_shift
        y_pos = dir_pt[1] + y_dir_shift
        dist_0 = matrix[x_pos, y_pos]
        matrix[x_pos, y_pos] = d if (dist_0 == 0) or (dist_0 > d) else dist_0
    ...
    shape = ((2 * max_reflections + 1) * dimensions[0], (2 * max_reflections + 1) * dimensions[1])
    matrix_pos_trainer = np.zeros(shape)
    matrix_pos_me = np.zeros(shape)
    ...
    for i in range(-max_reflections, max_reflections + 1, 1):
        for j in range(-max_reflections, max_reflections + 1, 1):
            ...
            if dist_trainer < distance:
                ...
                append_direction_with_dist(dir_trainer, matrix_pos_trainer, dist_trainer)
            ...
            if dist_my_refct < distance:
                ...
                append_direction_with_dist(dir_my_refct, matrix_pos_me, dist_my_refct)

    # get the direction which didn't target me before the trainer
    matrix_pos_trainer = (matrix_pos_trainer < matrix_pos_me) * (matrix_pos_trainer != 0) + \
                         (matrix_pos_trainer > 0) * (matrix_pos_me == 0)

    count_shots = int(matrix_pos_trainer.sum())

    # for the case with only direct shot
    if (count_shots == 0) and (get_distance(your_position, trainer_position) <= distance):
        return 1
